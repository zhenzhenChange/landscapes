...
这个函数需要一个参数，我们将其命名为 $x$。在函数体中描述了输入 $x$ 和输出之间的关系，它将在 $x$ 的任何数值上加 $1$，并返回该结果。当我们将这个函数应用于一个值，如 $1$，我们将该值替换给 $x$：
...
抽象由两部分组成：头部和主体。函数的头部是一个 λ，后面跟一个变量名。该函数的主体是另一个表达式。因此，一个简单的函数可能会是这样的：

$$λx . x$$

在头部命名的变量是参数，并在函数体中绑定该变量的所有实例。这意味着，当我们将这个函数应用于一个参数时，函数体中的每一个 $x$ 都将具有该参数的值。我们将在下一节中演示这一点。

在上一节中，我们谈论的是名为 $f$ 的函数，但 λ 抽象 $λx . x$ 没有名字，它是一个匿名函数。一个具名函数可以被另一个函数通过名称进行调用，匿名函数则不行。

让我们来分解一下基本结构，将 $λx . x$ 分解成两部分：「 $λx .$ 」和 「 $x$ 」。

1. 「 $λx .$ 」是 λ 公式的头部范围，其中的 $x$ 是函数的单一参数，这将绑定到函数体中任何具有相同名称的变量。
...
我们将用一个数字做我们的第一个 β 还原。将上面的函数应用于 $2$，用 $2$ 代替函数体中的每个约束变量，并消除头部：
...
唯一的约束变量是单一的 $x$，因此将此函数应用于 $2$，返回 $2$。这个函数是恒等函数，它所做的只是接受一个参数 $x$，并返回同一个参数。$x$ 没有固定的含义，但是，由于它被绑定在这个函数的头部，当这个函数被应用于一个参数时，函数体中所有 $x$ 的实例必须具有相同的值。

让我们使用一个例子，将一些算术混合到我们的 λ 演算中。我们在这里用括号来说明，主体表达式是 $x + 1$。换句话说，我们不是将函数应用于 $1$：

$$(λx . x + 1)$$

如果我们把这个抽象概念应用于 $2$，结果会是什么？对于 $10$ 又是如何呢？

β 还原是这样一个过程：将一个 λ 公式应用于一个参数，用参数的值替换绑定的变量，并消除头部，消除头部告诉你函数已被应用。

我们也可以将我们的恒等函数应用到另一个 λ 抽象中：

$$(λx . x)(λy . y)$$

在这种情况下，我们会把整个抽象概念替换成 $x$。我们在这里使用一种新的语法，即 $[x := z]$，表示 $z$ 将被替换为所有出现的 $x$（这里 $z$ 是函数 $λy . y$）。
...
我们的最终结果是另一个恒等函数。没有任何参数可以应用它，所以我们没有任何东西可以还原。

再来一次，但这一次我们将添加另一个参数：

$$(λx . x)(λy . y)z$$

λ 演算中的应用是左关联的。也就是说，除非有特定的括号提示，否则它们都是在左边关联的，或者说是分组的。
...
我们不能再简化了，因为已经没有东西可以应用了，而且我们对 $z$ 一无所知。我们将在下面看到有多个头的函数，还有自由变量（即函数体中的变量不受头部的约束），但基本过程将保持不变。当没有更多的头部或 λ 需要应用，或者没有更多的参数需要应用函数时，β 还原的过程就会停止。因此，一个计算由一个初始的 λ 表达式（或者两个，如果你想把函数和它的输入分开的话）加上一个有限的 λ 项序列组成，每个项都是由前一个项通过一次 β 还原的应用推导出来的。我们继续遵循应用规则，用参数代替绑定的变量，直到没有更多的头部可以执行或没有更多的参数可以应用。
...
|                `恒等` | `identity`                 |
|            `一等公民` | `first-class`              |
|              `左关联` | `left associative`         |
|              `右关联` | `right associative`        |
